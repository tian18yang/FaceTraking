...
while runLoop
    ...
    % Get the next frame.
    videoFrame = fliplr(permute(snapshot(cam),[2,1,3]));
    ...
    if numPts < 10
        ...
        if ~isempty(bbox)
            ...
            xy = [(bboxPoints(1,1)+bboxPoints(3,1))/2,(bboxPoints(1,2)+bboxPoints(3,2))/2];
        ...
        if numPts >= 10
            ...
            xy = [(bboxPoints(1,1)+bboxPoints(3,1))/2,(bboxPoints(1,2)+bboxPoints(3,2))/2];
            sizebbox = [(bboxPoints(1,1)-bboxPoints(3,1))];
    ...
    Text = [strcat('x mid = ',num2str(xy(1))) , strcat('y mid = ',num2str(xy(2)))];
    videoFrame = insertText(videoFrame,[100,200],Text,'FontSize',30);
